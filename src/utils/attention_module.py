...
class TransformerBlock(Layer):
	...
	def __init__(
		...
		self.mha_ln = LayerNormalization(axis=-1)
		self.mha = MultiheadAttention(**attention_kwargs)
		self.mha_dropout = Dropout(dropout_rate)

		self.mlp_ln = LayerNormalization(axis=-1)
		self.mlp_linear1 = Dense(channels * 2)
		self.mlp_dropout1 = Dropout(dropout_rate)
		self.mlp_linear2 = Dense(channels)
		self.mlp_dropout2 = Dropout(dropout_rate)
...
class MultiheadAttention(Layer):
	...
	def __init__(self,
		...
		self._v_layer = Dense(
			...
			use_bias=False,
			kernel_initializer=self._initializer)
		w_init = initializers.Zeros() if zero_initialize else self._initializer
		self._embedding_layer = Dense(
			embedding_size,
	...
	def get_config(self):
		...
		# 		"d_model": self._key_size,
		# 		"wq": self._q_layer.w,
		# 		"wk": self._k_layer.w,
		# 		"wv": self._v_layer.w,
		# 		"wemb": self._embedding_layer.w,
		# 	}
		# )
		return config
...
def example():
	...
	return transformer_block(tf.ones((200, 24, 10 * 1)), is_training=True)


if __name__ == '__main__':
	out = example()
# tfb = TransformerBlock(channels=16, dropout_rate=0.5, attention_kwargs=dict(
# 	value_size=4,
# 	key_size=16,
# 	num_heads=4,
# 	relative_positions=True,
...
# 	attention_dropout_rate=0.05,
# ))
# out = tfb(tf.ones((2, 24, 10)), is_training=True)
